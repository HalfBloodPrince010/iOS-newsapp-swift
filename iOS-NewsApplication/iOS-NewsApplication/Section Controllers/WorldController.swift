...
class WorldController: UIViewController, IndicatorInfoProvider, NewsModelDelegate, UIContextMenuInteractionDelegate {
        ...
        func handleBookmark(cell:UITableViewCell){
            ...
            if isThere == false{
                if var write_bookmark = UserDefaults.standard.array(forKey: "Bookmarks") as Array?{
                    ...
                    print("Bool False 1")
                    print(bookmark_data!)
                    let ucell = worldTableView.cellForRow(at: newsClicked!) as! SectionCell
    ...
    @objc func refreshPage(refreshControl: UIRefreshControl) {
        print("Hello World!")
        self.newsData.getSectionHeadlines(section: "world")
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
            print("Inside disSelectFunction")
    ...
    let bookmark = UIAction(title: "Bookmark", image: UIImage(systemName: "bookmark")) { _ in
        let ucell = self.worldTableView.cellForRow(at: cellRow) as! SectionCell
...
extension WorldController : UITableViewDataSource, UITableViewDelegate{
